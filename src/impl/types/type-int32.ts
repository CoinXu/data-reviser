...
const Def: Templates = {
  ...
  // { key, value, limit }
  gt: "type int32 must less than {{limit}} but got {{value}}",
  // { key, value, limit }
  lt: "type int32 must great than {{limit}} but got {{value}}"
};

function TypeInt32(template?: string | Templates): PropertyDecorator {
  const Temps = {
  ...
  function decorator(target: any, key: string, value: any, options: ReviserDecoratorOptions): ReviserDecoratorReturns<{}> {
    ...
    if (value > IEEE754Limits.Int32.Max) {
      return parse(Temps.gt, { key, value, limit: IEEE754Limits.Int32.Max });
    }

    if (value < IEEE754Limits.Int32.Min) {
      return parse(Temps.lt, { key, value, limit: IEEE754Limits.Int32.Min });
    }
